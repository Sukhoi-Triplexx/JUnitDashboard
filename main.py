...
WATCHED_FOLDER = os.environ.get('WATCHED_FOLDER', os.path.join(APP_DIR, 'watched_reports'))

print(f"APP_DIR: {APP_DIR}")
print(f"WATCHED_FOLDER: {WATCHED_FOLDER}")
print(f"BACKUP_DIR: {os.path.join(APP_DIR, 'backups')}")
...
def save_group_to_db(group_data, group_id, is_auto=False):
    ...
    # Используем дату из group_data, если есть
    if group_data.get('created_date_str'):
        created_date_str = group_data['created_date_str']
        created_date = group_data.get('created_at', datetime.now().isoformat())
    else:
        now = datetime.now()
...
def process_file(filepath):
    try:
        ...
        # Извлекаем дату из XML (первый testcase или testsuite)
        test_date = None
        try:
            tree = ET.parse(filepath)
            root = tree.getroot()
            
            # Ищем timestamp в testsuite
            suite = root.find('testsuite') if root.tag == 'testsuites' else root
            if suite is not None:
                timestamp = suite.get('timestamp')
                if timestamp:
                    test_date = timestamp
            # Если нет, ищем в первом testcase
            if not test_date:
        ...
        if not test_date:
            ...
            print(f"  ⚠️ No timestamp in XML, using current time")
        else:
            print(f"   Test date from XML: {test_date}")
        ...
        if USE_POSTGRES:
            cursor.execute('SELECT group_id, file_path FROM processed_files WHERE file_hash = %s', (file_hash,))
        else:
            cursor.execute('SELECT group_id, file_path FROM processed_files WHERE file_hash = ?', (file_hash,))
        
        existing = cursor.fetchone()
        
        if existing:
            existing_group_id = existing[0]
            existing_path = existing[1]
            print(f"  ⏭ Duplicate content! Already in: {os.path.basename(existing_path)}")
            return_db_connection(conn)
            return existing_group_id
        
        # Генерируем group_id с датой из XML
        try:
            # Парсим дату для group_id
            if 'T' in test_date:
                date_part = test_date.split('T')[0].replace('-', '')
            else:
                date_part = test_date.replace('-', '')[:8]
        except:
            date_part = datetime.now().strftime('%Y%m%d')
        
        group_id = f"auto_{date_part}_{filename}"
        
        data['filename'] = filename
        group_data = {
            'reports': [data],
            'combined_summary': data['summary'],
            'created_at': test_date,  # Используем дату из XML!
            'created_date_str': test_date[:10] if test_date else datetime.now().strftime('%Y-%m-%d'),
            'file_count': 1,
            'is_auto': True,
            'display_name': f"{filename} ({test_date[:19] if test_date else 'unknown'})"
        }
        ...
        if USE_POSTGRES:
            ...
            ''', (filepath, file_hash, datetime.now().isoformat(), group_id, file_size))
        else:
            cursor.execute('''
...
def scan_watched_folder():
    ...
    try:
        ...
        for file_info in all_files:
            ...
            # Ищем в БД записи с таким же путём ИЛИ с таким же хешем
            if USE_POSTGRES:
                cursor.execute('''
                    SELECT file_path, file_hash, group_id 
            ...
            if not existing:
                ...
                if existing_hash != file_hash:
                    # Хеш разный — содержимое изменилось
                    files_to_process.append(file_info)
                    print(f"  🔄 Changed file: {filename} (content changed)")
                else:
                    # Хеш одинаковый — содержимое не изменилось
                    if existing_path != filepath:
                        print(f"  📁 Same content, different location: {filename} (skipping)")
                    else:
...
@app.route('/api/reports/compare-multi', methods=['POST'])
def compare_multiple_reports():
    """
    Сравнивает несколько отчётов между собой.
    Принимает список group_ids и возвращает матрицу сравнения.
    """
    try:
        data = request.json
        group_ids = data.get('group_ids', [])
        ...
        for group_id in group_ids:
            if USE_POSTGRES:
                cursor.execute('SELECT group_data, display_name, created_date FROM report_groups WHERE group_id = %s', (group_id,))
            else:
                cursor.execute('SELECT group_data, display_name, created_date FROM report_groups WHERE group_id = ?', (group_id,))
            ...
            if row:
                ...
                reports_data.append({
                    'group_id': group_id,
                    'display_name': row[1] if USE_POSTGRES else row[1],
                    'created_date': row[2] if USE_POSTGRES else row[2],
                    'data': group_data,
                    'tests': extract_all_tests(group_data)
                })
        
        return_db_connection(conn)
        
        if len(reports_data) < 2:
            return jsonify({'error': 'Not enough valid reports found'}), 400
        
        # Строим карту всех уникальных тестов
        all_tests_map = {}
        for report in reports_data:
            for test in report['tests']:
                key = f"{test['classname']}|{test['name']}"
                if key not in all_tests_map:
                    all_tests_map[key] = {
                        'name': test['name'],
                        'classname': test['classname'],
                        'reports': {}
                    }
                all_tests_map[key]['reports'][report['group_id']] = test['status']
        ...
        result = {
            ...
            'tests': [],
            'summary': {
                'total_unique_tests': len(all_tests_map),
                'stable_passed': 0,  # Всегда passed
                'stable_failed': 0,  # Всегда failed
                'flaky': 0,          # Меняется
                'new_in_latest': 0,  # Появились в последнем
                'removed_in_latest': 0  # Исчезли в последнем
            }
        }
        
        # Анализируем каждый тест
        latest_group_id = reports_data[-1]['group_id']
        previous_group_id = reports_data[0]['group_id'] if len(reports_data) > 1 else None
        
        for key, test_data in all_tests_map.items():
            statuses = list(test_data['reports'].values())
            unique_statuses = set(statuses)
            
            test_info = {
                'name': test_data['name'],
                'classname': test_data['classname'],
                'statuses': test_data['reports'],
                'unique_statuses': list(unique_statuses),
                'is_stable': len(unique_statuses) == 1,
                'is_flaky': len(unique_statuses) > 1,
                'latest_status': test_data['reports'].get(latest_group_id, 'unknown'),
                'previous_status': test_data['reports'].get(previous_group_id, 'unknown') if previous_group_id else None
            }
            
            # Статистика
            if test_info['is_stable']:
                if test_info['latest_status'] == 'passed':
                    result['summary']['stable_passed'] += 1
                else:
                    result['summary']['stable_failed'] += 1
            else:
                result['summary']['flaky'] += 1
            
            # Проверяем новые/исчезнувшие
            if previous_group_id:
                if test_info['latest_status'] != 'unknown' and test_info['previous_status'] == 'unknown':
                    result['summary']['new_in_latest'] += 1
                    test_info['is_new'] = True
                elif test_info['latest_status'] == 'unknown' and test_info['previous_status'] != 'unknown':
                    result['summary']['removed_in_latest'] += 1
                    test_info['is_removed'] = True
            
            result['tests'].append(test_info)
        
        # Сортируем: сначала flaky, потом стабильные
        result['tests'].sort(key=lambda x: (not x['is_flaky'], x['name']))
...
def extract_all_tests(group_data):
    """Извлекает все тесты из group_data"""
    tests = []
    reports = group_data.get('reports', [group_data])
    for report in reports:
        for suite in report.get('suites', []):
            for test in suite.get('test_cases', []):
                tests.append({
...
@app.route('/compare')
def compare_page():
    """Страница сравнения с поддержкой параметров base и compare"""
    base_id = request.args.get('base', '')
    compare_id = request.args.get('compare', '')
    return render_template('compare_multi.html', config=config, base_id=base_id, compare_id=compare_id)

@app.route('/api/reports/find-similar/<group_id>', methods=['GET'])
def find_similar_reports(group_id):
    """
    Находит отчёты, похожие на указанный, по набору тестов.
    Возвращает список отчётов с процентом совпадения.
    """
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        current_data = json.loads(result[0]) if isinstance(result[0], str) else result[0]
        current_tests = extract_all_tests(current_data)
        current_test_set = set([f"{t['classname']}|{t['name']}" for t in current_tests])
        
        # Получаем все остальные отчёты
        if USE_POSTGRES:
            cursor.execute('SELECT group_id, group_data, display_name, created_date FROM report_groups WHERE group_id != %s', (group_id,))
        else:
            cursor.execute('SELECT group_id, group_data, display_name, created_date FROM report_groups WHERE group_id != ?', (group_id,))
        ...
        for row in rows:
            other_data = json.loads(row[1]) if isinstance(row[1], str) else row[1]
            other_tests = extract_all_tests(other_data)
            other_test_set = set([f"{t['classname']}|{t['name']}" for t in other_tests])
            
            # Вычисляем пересечение
            intersection = current_test_set & other_test_set
            union = current_test_set | other_test_set
            
            similarity = len(intersection) / len(union) * 100 if union else 0
            
            # Только если есть хотя бы 30% совпадения
            if similarity >= 70:
                # Определяем тип сравнения
                if len(other_test_set) > len(current_test_set):
                    comparison_type = 'larger'  # Больше тестов
                elif len(other_test_set) < len(current_test_set):
                    comparison_type = 'smaller'  # Меньше тестов
                else:
                    comparison_type = 'similar'  # Похожее количество
                
                # Вычисляем изменения
                added = len(other_test_set - current_test_set)
                removed = len(current_test_set - other_test_set)
                
                similar_reports.append({
                    'group_id': row[0],
                    'display_name': row[2] if USE_POSTGRES else row[2],
                    'created_date': row[3] if USE_POSTGRES else row[3],
                    'similarity': round(similarity, 1),
                    'common_tests': len(intersection),
                    'total_tests': len(other_tests),
                    'added_tests': added,
        ...
        return jsonify({
            ...
            'similar_reports': similar_reports[:10]  # Топ-10
        })
